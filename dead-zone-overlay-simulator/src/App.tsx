...
export default function App() {
  ...
  const [stopButtonHoldTime, setStopButtonHoldTime] = useState<number>(0);
  const [emergencyHoldTime, setEmergencyHoldTime] = useState<number>(0);
  const [isPressingStop, setIsPressingStop] = useState<boolean>(false);
  const [isPressingEmergency, setIsPressingEmergency] = useState<boolean>(false);

  // Refs for Javascript timers
  const stopHoldIntervalRef = useRef<any>(null);
  const emergencyHoldIntervalRef = useRef<any>(null);

  useEffect(() => {
    return () => {
      if (stopHoldIntervalRef.current) clearInterval(stopHoldIntervalRef.current);
      if (emergencyHoldIntervalRef.current) clearInterval(emergencyHoldIntervalRef.current);
    };
  }, []);

  const startStopButtonHold = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    if (!serviceActive) {
    ...
    stopHoldIntervalRef.current = setInterval(() => {
      ...
      if (elapsed >= 10000) {
        ...
        setAllowedClicksMsg("✅ Daemon Stopped Safely (10s Hold Completed)!");
        if (stopHoldIntervalRef.current) {
          clearInterval(stopHoldIntervalRef.current);
          stopHoldIntervalRef.current = null;
        }
      } else {
        setStopButtonHoldTime(elapsed);
      }
    }, 100);
  };

  const endStopButtonHold = () => {
    if (stopHoldIntervalRef.current) {
      clearInterval(stopHoldIntervalRef.current);
      stopHoldIntervalRef.current = null;
    }
    if (isPressingStop) {
      if (stopButtonHoldTime < 10000) {
        setAllowedClicksMsg("⚠️ Prevented! Hold STOP button for 10 seconds!");
      }
      setIsPressingStop(false);
      setStopButtonHoldTime(0);
    }
  ...
  const startEmergencyHold = (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    setIsPressingEmergency(true);
    setEmergencyHoldTime(0);
    
    if (emergencyHoldIntervalRef.current) clearInterval(emergencyHoldIntervalRef.current);
    
    const startTime = Date.now();
    emergencyHoldIntervalRef.current = setInterval(() => {
      const elapsed = Date.now() - startTime;
      if (elapsed >= 10000) {
        setServiceActive(false);
        setDeadZoneLocked(false);
        setEmergencyHoldTime(0);
        setIsPressingEmergency(false);
        setAllowedClicksMsg("✅ Emergency Bypass Successful (10s Override)!");
        if (emergencyHoldIntervalRef.current) {
          clearInterval(emergencyHoldIntervalRef.current);
          emergencyHoldIntervalRef.current = null;
        }
      } else {
        setEmergencyHoldTime(elapsed);
      }
    }, 100);
  };

  const endEmergencyHold = (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    if (emergencyHoldIntervalRef.current) {
      clearInterval(emergencyHoldIntervalRef.current);
      emergencyHoldIntervalRef.current = null;
    ...
    if (isPressingEmergency) {
      if (emergencyHoldTime < 10000) {
        setAllowedClicksMsg("⚠️ Prevented accidental stop! Hold Emergency button for 10 seconds!");
      }
      setIsPressingEmergency(false);
      setEmergencyHoldTime(0);
    }
          ...
          <div className="sticky top-24 w-full flex flex-col items-center">
            ...
            <div className="relative bg-slate-950 ring-4 ring-slate-800 rounded-[38px] p-4 shadow-2xl border border-slate-700 max-w-[350px] w-full">
              ...
              >
                ...
                <div className="w-full h-full pt-7 pb-4 flex flex-col justify-between px-3 bg-slate-920">
                  ...
                  <div className="pt-2 flex flex-col gap-2.5 z-10">
                    ...
                    <div className="bg-slate-950 p-2 rounded-lg border border-slate-850">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            onMouseDown={startStopButtonHold}
                            onTouchStart={startStopButtonHold}
                            onMouseUp={endStopButtonHold}
                            onMouseLeave={endStopButtonHold}
                            onTouchEnd={endStopButtonHold}
                            className={`px-2 py-1 rounded text-[9px] font-extrabold flex items-center justify-center gap-1 transition-all relative overflow-hidden ${
                              !permissionGranted 
                                ? "bg-slate-800 text-slate-500 cursor-not-allowed"
                                : serviceActive
                                  ? "bg-red-500 hover:bg-red-600 text-white min-w-[105px]"
                                  : "bg-blue-600 hover:bg-blue-700 text-white"
                            }`}
                          >
                            {/* Inner progress bar fill when holding */}
                            {isPressingStop && serviceActive && (
                              <div 
                                className="absolute inset-y-0 left-0 bg-red-800 pointer-events-none transition-all duration-100"
                                style={{ width: `${(stopButtonHoldTime / 10000) * 100}%` }}
                              />
                            )}
                            <span className="relative z-10 flex items-center gap-1">
                              {serviceActive ? <Square className="w-2 h-2" /> : <Play className="w-2 h-2" />}
                              {serviceActive 
                                ? isPressingStop 
                                  ? "STOPPING" 
                                  : "STOP ENGINE" 
                      ...
                      <p className="text-[8px] text-slate-400 mt-1">Spawns structural overlay zones and floating emergency shortcuts.</p>
                    </div>
                ...
                {/* ================= FLOATING EMERGENCY DISABLE BUTTON ================= */}
                {permissionGranted && serviceActive && (
                  <div
                    style={{
                      position: 'absolute',
                    ...
                    onMouseDown={(e) => {
                      ...
                      if (simulatedScreenRef.current) {
                        const rect = simulatedScreenRef.current.getBoundingClientRect();
                        setEmergencyDragOffset({
                          x: (e.clientX - rect.left) - emergencyX,
                          y: (e.clientY - rect.top) - emergencyY
                        });
                      }
                      startEmergencyHold(e);
                    }}
                    onTouchStart={(e) => {
                      e.stopPropagation();
                      startEmergencyHold(e);
                    }}
                    onMouseUp={endEmergencyHold}
                    onTouchEnd={endEmergencyHold}
                    onMouseLeave={endEmergencyHold}
                    className={`w-14 h-14 rounded-full shadow-2xl border flex flex-col items-center justify-center text-[7px] text-white select-none transition-all relative overflow-hidden ${
                      isPressingEmergency
                        ? 'bg-red-800 border-yellow-400 ring-2 ring-yellow-400 scale-105'
                        : 'bg-red-600 hover:bg-red-700 border-red-500 cursor-grab active:scale-95 animate-pulse'
                    }`}
                    title="Drag to position. Hold for 10 seconds to stop dead zone."
                  >
                    {/* SVG circular progress ring inside emergency button */}
                    {isPressingEmergency && (
                      <div 
                        className="absolute inset-0 bg-red-950 pointer-events-none opacity-80 transition-all"
                        style={{ clipPath: `inset(${(1 - (emergencyHoldTime / 10000)) * 100}% 0px 0px 0px)` }}
                      />
                    )}
                    
                    <div className="relative z-10 flex flex-col items-center">
                      <Square className={`w-3.5 h-3.5 fill-white text-white ${isPressingEmergency ? 'text-yellow-300 fill-yellow-300' : ''}`} />
                      <span className="font-extrabold text-[6px] tracking-tighter uppercase pointer-events-none mt-0.5">
                        {isPressingEmergency ? "STOPPING" : "STOP"}
                      </span>
                    </div>
                  </div>
                )}
